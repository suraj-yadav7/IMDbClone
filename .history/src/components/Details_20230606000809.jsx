...
const MY_API=process.process.env.MY_API_KEY
const Details = () => {

  const {id} = useParams()
  const [genreData , setGenreData] = useState()
  console.log("The genre data", genreData[1].id)

  const fetchGenre = async() =>{
    await fetch(`https://api.themoviedb.org/3/genre/movie/list?api_key="${MY_API}"&language=en-US`).then((response)=>response.json()).then((data)=>setGenreData(data))
  }
                ...
                <div className="movie__detailRight">
                    <div className="movie__detailRightTop">
                        ...
                        <div className="movie__genres">
                            {
                                singleMovieData && singleMovieData.genres
                                ? 
